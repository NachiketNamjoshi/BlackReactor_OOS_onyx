...
#ifndef VENDOR_EDIT //shankai@bsp,change munu key to app_slectkey
    ...
    for (finger = 0; finger < fingers_to_process; finger++) {
        ...
        if (finger_status) {
            ...
            if ((TP_VKEY_NONE != get_virtual_key_button(x, y)) && !enable_virtual_key) {
                ...
                input_report_key(rmi4_data->input_dev,
                                 BTN_TOOL_FINGER, 1);
                //shankai@bsp ,  insert one point for control panel 2015-8-26
                if (old_y [finger]> 1919 && y < 1919)
                {
                    input_report_abs(rmi4_data->input_dev,
                                     ABS_MT_POSITION_X, x);
                    input_report_abs(rmi4_data->input_dev,
...
extern int synaptics_rmi4_get_firmware_version(int vendor_id,int lcd_type_id);

int lcd_type_id;
int tp_type_id;

char* synaptics_rmi4_get_vendorstring(int tp_type_id,int lcd_type_id) {
    char *pconst = "UNKNOWN";

    pconst = tp_firmware_strings[tp_type_id-1][lcd_type_id];

    sprintf(synaptics_vendor_str,"%s(%x)",pconst,
            synaptics_rmi4_get_firmware_version(tp_type_id ,lcd_type_id));

    return synaptics_vendor_str;
}
static int __init lcd_type_id_setup(char *str)
{
    if(0== strcmp("1:dsi:0:qcom,mdss_dsi_jdi_1080p_cmd",str))
        lcd_type_id = LCD_VENDOR_JDI;

    else if (0== strcmp("1:dsi:0:qcom,mdss_dsi_truly_1080p_cmd",str))
        lcd_type_id = LCD_VENDOR_TRULY;

    else if(0== strcmp("1:dsi:0:qcom,mdss_dsi_sharp_1080p_cmd",str))
                                ...
                                const struct i2c_device_id *dev_id)
{
    ...
    //add by shankai for device info
